...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    // Fetch verified status (20+ followers)
    $stmt = $connection->prepare("SELECT COUNT(*) AS followers_count FROM followers WHERE followed = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $followersData = $stmt->get_result()->fetch_assoc();
...
<section class="main_content">
  <?php if (isset($_SESSION['add_post_success'])) : ?>
    <div class="alert_message success" id="alert_message">
      <p><?= $_SESSION['add_post_success']; unset($_SESSION['add_post_success']); ?></p>
    </div>
  <?php elseif (isset($_SESSION['edit_profile_success'])) : ?>
    <div class="alert_message success" id="alert_message">
      <p><?= $_SESSION['edit_profile_success']; unset($_SESSION['edit_profile_success']); ?></p>
    </div>
  <?php elseif (isset($_SESSION['follow'])) : ?>
    <div class="alert_message error" id="alert_message">
      <p><?= $_SESSION['follow']; unset($_SESSION['follow']); ?></p>
    </div>
  <?php elseif (isset($_SESSION['follow_success'])) : ?>
    <div class="alert_message success" id="alert_message">
      <p><?= $_SESSION['follow_success']; unset($_SESSION['follow_success']); ?></p>
    </div>
  <?php endif ?>
